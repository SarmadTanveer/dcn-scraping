...
#Change
data = readList('tritan-rest-links')
lenData = data['length']
data = data['data']
links=[]
for link in data:
...
while(x<lenData):
    ...
    for link in targetCertificates:
        certificates.append(parseCertificate(link))
        x+=1
    #change
    writeList('tritan-rest-certificates-'+str(y), {'length': len(certificates), 'data': certificates})
    y+= 1
